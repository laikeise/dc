...
{	
{	
	$filename="DepartureFlight_".$_SESSION["sUSERID"]."_".date("Ymd").".xls";
	/*if(isset($_POST['export_departure'])=="Export"){
		$filename="Departure".date("Ymd");
	...
	// Remove fields
        $fs = array("blacklist", "player_type","fullname","sex","dietary","hotelcheckindate","hotelcheckoutdate","day_stay","arrival_date","arrival_time","arrival_flight","arrival_port","room_short_form","depart_port","comment") ;
	...
	print "<tr><td colspan=".($tbcount)."><div align=center class=ftitle5>By Departure Date Report ";
	print "</div></td></tr>";
	ob_start();
	...
	if(($tmpstname))
	{
		if($tmpstname=="Arrive"){
			...
			$searchcol="depart_date ASC, depart_time ASC, depart_flight ASC";}
...
/*
		...
		$wherecol=$wherecol." a.depart_flight NOT LIKE \"%NA%\" and";
...
*/
	...
	if($qrow=mysql_fetch_array($resultx))
	{
		...
		$pgcount++;
		if(!isset($tempdate)){
			$tempdate=$qrow["depart_date"];
			}
		if(!isset($temptime)){
			$temptime=$qrow["depart_time"];
			}
		if(!isset($tempflight)){
			$tempflight=$qrow["depart_flight"];
		}
		print "<tr height=55 valign=bottom><td colspan=".($tbcount).">";
		print "<b>Date : ".strftime("%d-%b-%y", strtotime($tempdate))."</b><br/>";
		$tempnflight=explode(":~:",$tempflight);
		print "<b>Flight : ".$qrow["depart_flight"]."</b><br/>";
		print "<b>Airport : ".$AustGolf_Addon->get_airport_name($qrow["depart_port"])."</b><br/>";
		print "</td></tr>" ;
                print $tableheader;
	}
	...
	while($qrow=mysql_fetch_array($resultx))
	{
		...
		if($tempdate!=$qrow["depart_date"]){
			$tempdate=$qrow["depart_date"];
			$printerheader=1;
			}
		if($temptime!=$qrow["depart_time"]){
			$temptime=$qrow["depart_time"];
			$printerheader=1;
			}
		if($tempflight!=$qrow["depart_flight"]){
			$tempflight=$qrow["depart_flight"];
			$printerheader=1;
		}
		...
		if($printerheader==1)
		{
			...
			$tempnflight=explode(":~:",$tempflight);
			print "<b>Flight : ".$qrow["depart_flight"]."</b><br/>";
			print "<b>Airport : ".$AustGolf_Addon->get_airport_name($qrow["depart_port"])."</b></td></tr>";
			print $tableheader;
                        $g=1;
                        $pgcount++;
                        $_pg=0;
		...
		foreach ($tblist as $i => $colname) {
				...
				if($colname=="cust_id"){
                                        ...
                                        if($htmltxt!=="")
                                                $htmltxt=$htmltxt;
                                        $htmltxt="<div style=\"width:250px;height:65px;overflow-y:auto;\" class=fname1>$htmltxt</div>";
