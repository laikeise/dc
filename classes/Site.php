...
// +----------------------------------------------------------------------+
//
// $Id: Site.php,v 1.1 2006/04/20 08:11:05 devcvs Exp $
...
class Site extends Session {
    ...
    * @return string - the access lvl of the grp_id
    ...
    function get_TBName($controlid) {
    	//$temprow="";
    	$q="select tb_access from group_permission where grp_id='$controlid';";
    	//print "<br>".$q;
    	$result = $this->doSQL($q);
    	while($qrow=mysql_fetch_array($result)){
    		$temprow=((isset($temprow))?array_merge($temprow,$qrow["tb_access"]):$qrow["tb_access"]);
    	}
    	//print $temprow;
    	if($temprow)
    		return $temprow;
    ...
    * @return boolean
    ...
    function check_TBAction($controlid,$inttb,$intaction) {
    	...
    	{
    		$q="select tb_access from group_permission where grp_id='$controlid' and $intaction=1 and tb_access ='$inttb';";
    		//print_r($_SESSION);
    		$result = $this->doSQL($q);
    		//print $result;
    		if($result)
