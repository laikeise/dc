...
//
//Version 1.25: Supports Frames & solved the MAC OS 9.X 10 BUG in IE 
//
...
// var Layer="\"<div nowrap id='\"+IDLABEL+\"' class='CBORDER' Style='Position:absolute; visibility:\"+VISIBILITY+\"; left:\"+(LEFT+NS6X)+\"px; top:\"+(TOP+NS6X)+\"px; width:\"+(WIDTH+NS6X)+\"px; height:\"+(HEIGHT+NS6X)+\"px; background-color:\"+MyLayerColor+\"; z-index:\"+ZINDEX+\"' \"";
var Layer="\"<div id='\"+IDLABEL+\"' class='CBORDER' Style='cursor: hand; Position:absolute; visibility:\"+VISIBILITY+\"; left:\"+(LEFT+NS6X)+\"px; top:\"+(TOP+NS6X)+\"px; width:\"+(WIDTH+NS6X)+\"px; height:\"+(HEIGHT+NS6X)+\"px; background-color:\"+MyLayerColor+\"; z-index:\"+ZINDEX+\"' \"";
var Layer_End="</div>";
...
/******************************************/
function AddMenu(ID, Parent_ID, Text, Image, RollImage, URL, TARGET, OnClickAction)
{ 
  //Validation
...
function Build()
{
   ...
   {  
    ...
    document.writeln("<script type='text/javascript'>var clicked=0;</script>");
   ...
   for (var j=0;j<MaxMenuIndex;j++)
   {
     ...
     mytarget=""; 
     if(Menu[j][6]!=""){mytarget=Menu[j][6];}
     document.write(LAYER[j] + " onClick=\"javascript: winopen('" + Menu[j][5] + "','" + mytarget + "')\"" + " onMouseOver=\""+MOUSEOVERCODE+"\" onMouseOut=\""+MOUSEOUTCODE+"\">");
     ...
     if(Menu[j][5]!="")
     {
		...
		if(Menu[j][7])
			LINK=LINK+" onClick=\"javascript: "+Menu[j][7]+"\"";
     ...
     if(Menu[j][3]!="")
     {
       ImageCode=ImageCode+"<IMG align='absmiddle' Name='Image"+Menu[j][0]+"' SRC='"+Menu[j][3]+"' BORDER=0>";  
     } 
...
function ChangeWidth(){
    for (var k=0; k<MaxMenuIndex; k++){
		if(Menu[k][0]!=Menu[k][1]){
			IDLAYER="Layer"+Menu[k][0];
			MenuItemObj=findObj(IDLAYER);
			MenuItemObj.noWrap=true;
			MaxWidth=MenuItemObj.offsetWidth;

		    for (var k2=0; k2<MaxMenuIndex; k2++){
				if((Menu[k2][0]!=Menu[k2][1])&&(Menu[k2][1]==Menu[k][1])&&(Menu[k][0]!=Menu[k2][0])){
					CIDLAYER="Layer"+Menu[k2][0];
					CMenuItemObj=findObj(CIDLAYER);
					if(CMenuItemObj.offsetWidth > MaxWidth){
						MaxWidth=CMenuItemObj.offsetWidth;
					}
				}					
			}

		    for (var k2=0; k2<MaxMenuIndex; k2++){
				if((Menu[k2][0]!=Menu[k2][1])&&(Menu[k2][1]==Menu[k][1])){
					CIDLAYER="Layer"+Menu[k2][0];
					CMenuItemObj=findObj(CIDLAYER);
					CMenuItemObj.style.width=MaxWidth;
				}					
			}
...
//Use for generation delete window
function delWindow(val,nameff) 
{
 if (confirm( urldecode(nameff) + ' will be deleted! Are you sure you want to proceed?')) {
 	delWin=window.open('index.php?'+val,null,'resizable=yes,height=320,width=400,status=no,location=yes,fullscreen=no,menubar=no,scrollbars=yes,toolbar=no,directories=no,channelmode=no');
 	delWin.focus();
 }
 return false;
}
