...
?>
<br>
<div class="ftitle5" align="center">Winners from Previous Events</div>
<br>
<div align="center" class="fname1">
	<b>
<?
	$query = "SELECT `year` FROM `past_winners` GROUP BY `year` ORDER BY `year`";
...
?>
		<a href="?s=view_past_winners&tbn=past_winners&as=view&year=<? echo $row['year'] ?>">[ <? echo $row['year'] ?> ]</a>
<?
		}
	}
?>
...
<table border="1" cellspacing="0" cellpadding="2" align="center" width="55%">
	<tr>
		...
		<td class="ftitle1" width="20%">Year Won</td>
	</tr>
<?
	if (!isset($year)) $year = date('Y') - 1;
	$query = "SELECT * FROM `past_winners` WHERE `year` = '$year' ORDER BY `country`, `name`, `year`";
