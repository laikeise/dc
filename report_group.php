...
if(isset($_POST["View"])||$_POST['export_billing']=="Export")
{
	$filename="Billing_".$_SESSION["sUSERID"]."_".date("Ymd").".xls";
	//This is used to export the data into excel format by JOE-25 MAY 2010
	/*if(isset($_POST['export_billing'])&&$_POST['export_billing']=="Export")
	{
		$filename=$_POST['htmlgrpnum']."_Billing_".date("Ymd");
	...
	$r_tblist = array_flip($tblist);
        // Remove fields
	$fs = array("day_stay","fullname","sex","dietary","hotelcheckindate","hotelcheckoutdate","pickuptime",
		"arrival_date","arrival_time","arrival_flight","arrival_port","depart_date","depart_time","depart_flight","depart_port",
		"transport","transport2","room_short_form") ;
	...
	*/
	...
	ob_start();
	print "<tr><td colspan=".($tbcount+15)."><div align=center class=ftitle5>By Billing Group (". $grp_name[$_POST["htmlgrpnum"]] .")</div></td></tr>";
	print "<tr>";
	$tabcounter=0;
	while (list($key, $val) = each($tblist)) 
	{
			...
			/*
   			if($val=='arrival_date'||$val=='arrival_time'||$val=='arrival_flight'||$val=='depart_date'||$val=='depart_time'||$val=='depart_flight'||$val=='day_stay'||$val=='transport'||$val=='room_short_form')
   			{
				...
				//$tempshowitem=array_merge($tempshowitem,array($key=>1));
   				...
   				if(isset($_GET["st"]))
   				{
   					if($_GET["st"]=="$val"){
   						if(isset($_GET["od"])){
   							if($_GET["od"]=="asc"){
   								print  "<td valign=bottom class=ftitle1 rowspan=3><a href=\"index.php?".$sortlist."&st=$val&od=desc\">" . (!empty($htmldisrep["$val"])?$htmldisrep["$val"]:$val) . "</a>";
								print  "<img src=\"images/arrow_ASC.gif\">";
								$tabcounter++;}
							else{
								print  "<td valign=bottom class=ftitle1 rowspan=3><a href=\"index.php?".$sortlist."&st=$val&od=asc\">" . (!empty($htmldisrep["$val"])?$htmldisrep["$val"]:$val) . "</a>";
   					...
   					//print  "<td valign=bottom class=ftitle1 rowspan=3><a href=\"index.php?".$sortlist."&st=$val&od=desc\">" . (!empty($htmldisrep["$val"])?$htmldisrep["$val"]:$val) . "</a>";
   					print  "<td valign=bottom class=ftitle1 rowspan=3>" . (!empty($htmldisrep["$val"])?$htmldisrep["$val"]:$val) . "</a>";
   					$tabcounter++;}
	...
	<TR><!--ROWSPAN--><?  //this $tabcounter is number of records to display
		/*
			for($k=0;$k<$tabcounter;$k++){
				?><TD class=ftitle1 vAlign=top>&nbsp;</TD><?
			}
		*/
		$actdateresult= $User_Info->dosearchSQL("ref_act_tb","","act_date","inf","act_date","act_date");
...
//store the act_ref_id into array form use for seacrhing - joe
			$act_id = $actnumrow["act_ref_id"];
			$actarray[] = $act_id;
		}
		...
		*/
		//echo"<pre>"; var_dump($tarray); echo"<pre/>";
	print "</tr>";	
	...
	*/
	...
	if(!empty($tmpcolnum)&&!empty($tmpcolname))
	{
		...
		if($_POST["htmlgrpnum"]!="")
			$wherecol="  a.cust_id=b.cust_id and b.grp_id=\"".$_POST["htmlgrpnum"]."\"";
		else
			$wherecol="  a.cust_id=b.cust_id ";
		//$wherecol=($_SESSION["sRecord"]=="ALL"?"":("country_name=\"".$_SESSION["sRecord"]."\""));
	...
	while($qrow=mysql_fetch_array($resultx))
	{
		...
		/*
		if (($pgcount/10)==1)
			print "</table><table width='100%' border='1' cellspacing='0' cellpadding='0'>";
		else if (is_int($pgcount/10))
	...
	print "<div align=center class=ftitle5>By Billing Group (". $grp_name[$_POST["htmlgrpnum"]] .")</div>";
...
<form name="ViewGroup" method="post" action="">
	...
	<tr>
	<td>By Billing Group
	<select name="htmlgrpnum">
    	<option value="" ".($htmlgrpnum==""?"selected":"").">Select One</option><?php
// XXX Update billing groups -- marc 20080522
			print "<option value='MGM01' ".($htmlgrpnum=="MGM01"?"selected":"").">Thailand</option>";
			print "<option value='MGM02' ".($htmlgrpnum=="MGM02"?"selected":"").">Indonesia</option>";
			//print "<option value='MGM03' ".($htmlgrpnum=="MGM03"?"selected":"").">Korea</option>";
			print "<option value='MGM04' ".($htmlgrpnum=="MGM04"?"selected":"").">Malaysia</option>";
			print "<option value='MGM05' ".($htmlgrpnum=="MGM05"?"selected":"").">Singapore</option>";
			print "<option value='MGM06' ".($htmlgrpnum=="MGM06"?"selected":"").">Sri Lanka</option>";
			print "<option value='MGM07' ".($htmlgrpnum=="MGM07"?"selected":"").">Taiwan</option>";
			print "<option value='MGM08' ".($htmlgrpnum=="MGM08"?"selected":"").">Vietnam</option>";
			print "<option value='MGM09' ".($htmlgrpnum=="MGM09"?"selected":"").">Australia</option>";
			print "<option value='MGM10' ".($htmlgrpnum=="MGM10"?"selected":"").">Germany</option>";
			print "<option value='MGM11' ".($htmlgrpnum=="MGM11"?"selected":"").">Philippines</option>";			
			print "<option value='GRP02' ".($htmlgrpnum=="GRP02"?"selected":"").">Admins</option>";
?></select></td>
	</tr>
	<tr>
		<td>Rows per page:&nbsp;&nbsp;<input type="text" size="4" maxlength="4" name="perpage" value="8">
			<input type="submit" name="View" value="Submit">
		</td>
	</tr>
<!--<tr>Additional button to export the report into excel format
	<td>Export this Report to Excel:&nbsp;<input type="submit" name="export_billing" value="Export"/></td>
</tr>-->
</form>
	</table>
</div>
